(function explicitApplyRoot(window, document) {
  ...
  window.Vue.createApp({
    ...
    template: `<footer class="explicit-apply-footer" data-testid="settings-apply-footer"><p v-if="message" data-testid="settings-apply-status" role="status" aria-live="polite">{{ message }}</p><button type="button" data-testid="settings-apply" class="explicit-apply-button" :disabled="disabled" :aria-busy="phase === 'applying' || phase === 'pending'" @click="apply"><span v-if="phase === 'applying' || phase === 'pending'" class="spinner" aria-hidden="true"></span>{{ label }}</button></footer>`,
  }).mount(mount);
