...
module.exports = async function testLayoutPreviewAndMeasurementsInspector(assert) {
  ...
  const inspectorHeaderRule = css.match(/\.inspector-header\s*\{([^}]*)\}/)[1];
  assert(!/\bborder(?:-(?:top|right|bottom|left))?\s*:/.test(inspectorHeaderRule), "the lower header itself must not duplicate the overlay edge");
  assert(/\.inspector-header::before\s*\{[^}]*inset:\s*0[^}]*z-index:\s*10[^}]*border-radius:\s*var\(--control-radius\) var\(--control-radius\) 0 0[^}]*box-shadow:\s*inset 0 1px 0 var\(--line-strong\),\s*inset 1px 0 0 var\(--line\),\s*inset -1px 0 0 var\(--line\)[^}]*pointer-events:\s*none/.test(css), "one overlay must paint a stable full-width rounded upper and side edge inside the header pixels");
  assert(/\.inspector-header\s*>\s*\.inspector-tabs\s*\{[^}]*height:\s*100%[^}]*overflow-x:\s*hidden[^}]*overflow-y:\s*hidden/.test(css), "the lower tabs must inherit one header height without duplicated min/max declarations");
  assert(/\.inspector-header\s*>\s*\.inspector-tabs button\s*\{[^}]*height:\s*100%[^}]*line-height:\s*20px/.test(css), "lower tab buttons must inherit the same single height");
