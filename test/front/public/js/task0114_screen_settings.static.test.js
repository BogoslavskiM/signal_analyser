...
module.exports = async function testTask0114ScreenSettings(assert) {
  ...
  assert(/linkTime \? null : actual\("time\.x_limits"\)/.test(settings) && /linkAmplitude \? null : actual\("time\.y_limits"\)/.test(settings) && /group\("ranges", "Диапазоны"/.test(settings), "linked limits must disappear from the single active-Area Ranges group instead of leaving duplicate sections");
  assert(/context\.linkPreview \? context\.linkPreview\.linkTime : booleanValue\("time\.link_time"\)/.test(settings) && /setLinkPreview:function \(linkTime, linkAmplitude, linkFrequency, linkMagnitude\)/.test(settings), "Area inventory must follow the unsaved Screen link preview immediately");
  assert(/function previewScreenLinks\(draft\)[\s\S]*?settings\.setLinkPreview\(draft && draft\.linkTime, draft && draft\.linkAmplitude, draft && draft\.linkFrequency, draft && draft\.linkMagnitude\)/.test(app) && /dataset\.screenLinkTime[\s\S]*?renderScreenSettings\(activeDisplay\(\)\)[\s\S]*?scheduleScreenSettingsApply/.test(app) && /dataset\.screenLinkAmplitude[\s\S]*?renderScreenSettings\(activeDisplay\(\)\)[\s\S]*?scheduleScreenSettingsApply/.test(app) && /dataset\.screenLinkFrequency[\s\S]*?renderScreenSettings\(activeDisplay\(\)\)[\s\S]*?scheduleScreenSettingsApply/.test(app), "all Screen link checkboxes must refresh the shared preview before Apply");
  assert(/var parameterRows=\(draft\.linkTime \? settings\.renderRows\(\["time\.units"\]\)/.test(app) && /var rangeRows=draft\.linkTime \? settings\.renderRows\(\["time\.x_limits"\]\) \+ screenRangeSlider\("time\.x_limits", "x", draft\)/.test(app) && /screenSettingsGroup\("ranges", "Диапазоны", rangeRows\)/.test(app), "linked Screen limits must share one Ranges group while time/frequency units stay under Parameters");
  ...
  assert(/function keepAutomaticRangeInputsEmpty\(fieldId, axis, draft\)[\s\S]*?minimum\.value=current\.min == null \? minimumIntent == null \? "" : minimumIntent : String\(current\.min\);[\s\S]*?maximum\.value=current\.max == null \? maximumIntent == null \? "" : maximumIntent : String\(current\.max\);/.test(app), "automatic wire bounds must stay empty unless the current frontend-only viewport intent supplies a visible value");
