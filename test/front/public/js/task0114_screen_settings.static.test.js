...
module.exports = async function testTask0114ScreenSettings(assert) {
  ...
  assert(/function applySettings\(\)[\s\S]*?postLayout\(\{ operation:"resize"[\s\S]*?skipSettings:true, skipOutput:true[\s\S]*?persistLayoutLinks\(draft\)[\s\S]*?return settings\.flush\(\);[\s\S]*?needsSettingsApply \? applyLatest\(0\)/.test(app), "the shared Area/Screen Apply must serialize layout then exactly one settings flush before Apply");
  ...
  assert(/function areaScreenApplyState\(draft\)[\s\S]*?settings\.state\(\)[\s\S]*?settings\.stateFor\(screenLimitFieldIds\(draft\)\)/.test(app) && !/Promise\.all\(\[settings\.flush\(\), settings\.flushFields/.test(app), "the shared Apply state must combine Area fields with only currently visible Screen-linked limit fields without duplicate flushes");
