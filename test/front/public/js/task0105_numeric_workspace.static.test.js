...
module.exports = async function(assert) {
  ...
  const catalogService = fs.readFileSync(path.join(root, "lib/services/workspace_catalog_service.jl"), "utf8");
  assert(/function load_workspace_catalog!\([\s\S]*?enumeration = workspace_catalog_enumeration\(service\)[\s\S]*?workspace_catalog_snapshot\([\s\S]*?workspace_catalog_registry_store/.test(catalogService) && !/function latest_workspace_catalog!|!refresh && !isempty\(service\.registry\.snapshots\)/.test(catalogService), "each catalog read must enumerate fresh workspace variables and only store the resulting snapshot for safe follow-up import");
  assert(/workspaceVariables: function \(\) \{[\s\S]*?\.\/api\/workspace\/variables"/.test(api) && !/workspaceVariables: function \(refresh\)|workspace\/variables\?refresh/.test(api), "the browser catalog API must not offer a cached versus refresh request mode");
  assert(/openSignalAddDialog\(trigger\)[\s\S]*?loadSignalAddCatalog\(\)/.test(app) && !/signalAddCatalogFresh|signalAddCachedOpen|loadSignalAddCatalog\(false\)/.test(app), "every dialog opening must discard any catalog cache and issue a fresh load");
