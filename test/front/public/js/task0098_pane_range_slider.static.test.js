...
module.exports = async function testTask0098PaneRangeSliderStatic(assert) {
  ...
  assert(/function bindRangeSliderDoubleClick\([\s\S]*\.rangeslider-container[\s\S]*rangeSliderDataRangeByPane[\s\S]*"xaxis\.range\[0\]"[\s\S]*"xaxis\.autorange":false[\s\S]*addEventListener\("pointerdown"[\s\S]*previousPointerDown/.test(app), "two nearby pointer-down events on the horizontal Plotly slider must restore the already projected full range without a competing autorange pass");
  assert(/function resetGraphRange\([\s\S]*plotAutoscaleController\(\)[\s\S]*plotAutoscaleByPane\[runtimeKey\][\s\S]*autoscale\.relayout\(resetSnapshot\)[\s\S]*stopImmediatePropagation[\s\S]*Plotly\.relayout\(host, update\)/.test(app), "the first graph double-click must restore the exact captured plot baseline and suppress Plotly's later default autorange");
