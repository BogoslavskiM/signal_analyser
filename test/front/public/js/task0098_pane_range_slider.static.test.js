...
module.exports = async function testTask0098PaneRangeSliderStatic(assert) {
  ...
  assert(/function bindRangeSliderDoubleClick\([\s\S]*\.rangeslider-container[\s\S]*"xaxis\.range"\s*:\s*null[\s\S]*"xaxis\.autorange"\s*:\s*true[\s\S]*addEventListener\("pointerdown"[\s\S]*previousPointerDown/.test(app), "two nearby pointer-down events on the horizontal Plotly slider must bypass Plotly's swallowed pointer-up and delegate the default X range to autorange");
