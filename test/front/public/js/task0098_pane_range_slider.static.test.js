...
module.exports = async function testTask0098PaneRangeSliderStatic(assert) {
  ...
  assert(/function resetGraphRange\([\s\S]*rangeSliderDataRangeByPane[\s\S]*amplitudeDataRangeByPane[\s\S]*stopImmediatePropagation[\s\S]*Plotly\.relayout\(host, update\)/.test(app), "the first graph double-click must atomically restore projected X and Y ranges and suppress Plotly's later default autorange");
