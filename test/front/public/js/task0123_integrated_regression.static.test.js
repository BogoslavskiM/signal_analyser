...
module.exports = async function task0123IntegratedRegression(assert) {
  ...
  assert(/model\.pendingMainSignal = signalName;\s*setSignalTableMutationBusy\(true, signalName\);[\s\S]*?\.then\(function \(snapshot\) \{[\s\S]*?model\.pendingMainSignal = "";\s*setSignalTableMutationBusy\(false, ""\);[\s\S]*?\.catch\(function \(error\) \{[\s\S]*?model\.pendingMainSignal = "";\s*setSignalTableMutationBusy\(false, ""\);/.test(app), "main selection must keep the temporary pin continuous and clear it only after accepted or failed completion");
  ...
  assert(/var newer=\(editor\.intent \|\| 0\) > submittedIntent \|\| editor\.saveQueued;[\s\S]*?editor\.dirty=newer;[\s\S]*?editor\.applying=false;[\s\S]*?render\(\);[\s\S]*?if \(newer\) scheduleSignalMetadataSave\(\);/.test(metadata), "an accepted metadata snapshot may clear dirty only when no newer intent exists; queued input must remain dirty and autosave again");
