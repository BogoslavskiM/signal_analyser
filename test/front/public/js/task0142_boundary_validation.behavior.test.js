...
module.exports=async function(assert) {
  ...
  assert(/clearTimeout\(context\.timers\[item\.id\]\); delete context\.timers\[item\.id\];[\s\S]*?draft\.error = validation && validation\.message/.test(settings)&&/if \(!item \|\| item\.pseudo \|\| item\.visible === false \|\| !context\.displayId \|\| !context\.drafts\[item\.id\] \|\| context\.drafts\[item\.id\]\.error\) return Promise\.resolve\(\);/.test(settings),"an invalid draft must cancel its debounce and never reach updateSetting");
