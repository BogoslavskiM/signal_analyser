...
module.exports=async function(assert){
 ...
 assert(h.baseColumns.map(x=>x.id).join(",")==="sample_index,time,value"&&h.optionalColumns.map(x=>x.id).join(",")==="magnitude,square,square_root,signed_square_root_magnitude","base and four calculated sample columns must be exact");
 const all=h.visibleColumns(h.defaultVisibility()); assert(all.length===7&&all.every((x,i)=>x.id===["sample_index","time","value","magnitude","square","square_root","signed_square_root_magnitude"][i]),"all calculated columns must be initially visible in stable order");
 const off=h.toggle(h.defaultVisibility(),"square_root"); assert(!off.square_root&&h.visibleColumns(off).length===6&&h.rowProjection({square_root:17,signed_square_root_magnitude:-4},h.defaultVisibility()).find(x=>x.id==="square_root").value===17,"toggle must be frontend-only projection of provider fields, never browser math");
 ...
 assert(/\.sample-table \{ table-layout: auto; \}/.test(css)&&/square_root/.test(css)&&/signed_square_root_magnitude/.test(css),"calculated columns require compact table CSS widths");
};
