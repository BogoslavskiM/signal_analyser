...
module.exports = async function testMultiLayoutStaticContract(assert) {
  ...
  assert(layoutCss.includes(".pane-plot-host .modebar,.pane-plot-host .modebar-container") && layoutCss.includes(".graph-help-overlay{position:absolute;z-index:var(--layer-graph-help)") && !/\b(?:compact-legend|plot-legend)\b/.test(`${source}\n${layoutCss}`) && source.includes("showlegend:legendVisibilityByDisplay[activeDisplayId] !== false") && source.includes('legend:{ orientation:"v", x:.99, y:.99, xanchor:"right", yanchor:"top", font:{ family:"Roboto, Arial, sans-serif", size:12 }, bgcolor:"rgba(255,255,255,.86)", borderwidth:0 }'), "modebar and graph-help overlays must preserve a live viewport while Show Legend uses the native vertical Plotly legend, not a custom overlay");
  ...
  assert(!/(?:html\s*,\s*body|html,body|\.signal-analyser)\s*\{[^}]*(?:(?<!min-)(?:width|height|max-width|max-height)\s*:\s*\d+(?:\.\d+)?px)/i.test(layoutCss) && compactCss.includes(".main-stage { grid-template-columns: minmax(0, 1fr) 370px; overflow: hidden;"), "readable minimum dimensions are allowed, but page/app shells must not use fixed or max pixel canvas locks; components own overflow");
