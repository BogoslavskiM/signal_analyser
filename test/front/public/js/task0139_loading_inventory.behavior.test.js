...
module.exports=async function(assert){
  ...
  const intended=["abs","square","sqrt","signed_sqrt_abs","multiply","fft","custom"].map(value=>({value}));
  assert(inventory.withoutFft(intended).map(item=>item.value).join(",")==="abs,square,sqrt,signed_sqrt_abs,multiply,custom","signal operation list must remove FFT only and preserve the other intended operations");
  assert(/var options=\["abs", "square", "sqrt", "signed_sqrt_abs", "multiply", "fft", "custom"\][\s\S]*?operationInventory\.withoutFft\(options\)/.test(app),"operation dialog must pass its exact operation list through the FFT exclusion boundary before rendering");
