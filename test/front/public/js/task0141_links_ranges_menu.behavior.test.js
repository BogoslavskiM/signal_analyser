...
module.exports=async function(assert){
 ...
 assert(/screen-range-slider[\s\S]*?data-screen-range-input='min'[\s\S]*?data-screen-range-input='max'/.test(app)&&/keepAutomaticRangeInputsEmpty\([\s\S]*?rangeBoundaryIntent/.test(app)&&/resetRangeLifecycle\(display\.id,target\.paneId,\[target\.descriptor\]\)/.test(app),"all applicable Area ranges must reuse dual handles, retain blank automatic endpoints, and reset via the autorange lifecycle");
