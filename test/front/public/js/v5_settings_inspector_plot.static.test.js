...
module.exports = async function testV5SettingsInspectorAndPlotContracts(assert) {
  ...
  ["inspector-tab-signals", "inspector-tab-measurements", "inspector-tab-peaks", "inspector-pane-signals", "signal-rows", "signal-columns-menu"].forEach((id) => {
  ...
  assert(/model\.inspectorPage !== "signals"[\s\S]*data-testid='inspector-pane-/.test(app), "each non-signal bottom tab must render actual inspector pane content");
