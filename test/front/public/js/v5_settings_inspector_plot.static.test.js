...
module.exports = async function testV5SettingsInspectorAndPlotContracts(assert) {
  ...
  assert(/context\.page === "time"/.test(inventory) && !/context\.page === "measurements"/.test(inventory) && /context\.plotType/.test(inventory), "settings inventory must retain only graph settings pages and plot-type filtering");
