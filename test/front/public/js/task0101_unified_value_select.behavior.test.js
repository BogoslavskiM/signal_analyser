...
function createComponentHarness(source, filename) {
  ...
  function node(extra) {
    const value = Object.assign({
      ...
      classList: classList(), parentElement: null, children: [], value: "", title: "",
      ...
      matches(selector) { return selector === "[data-value-select-search]" && this.dataset.valueSelectSearch !== undefined; },
  ...
  const search = node({ dataset: { valueSelectSearch: "" } });
  ...
  Object.defineProperty(popup, "innerHTML", {
    ...
    set(markup) {
      ...
      search.value = "";
      this.children = markup ? [search, optionsHost] : [];
    }
  });
  popup.querySelector = function (selector) {
    if (selector === "[data-value-select-search]") return search;
  ...
  popup.contains = function (candidate) { return candidate === this || candidate === search || candidate === optionsHost || optionsHost.children.includes(candidate); };
  ...
  return { window, document, popup, search, optionsHost, triggers, successor, trigger, dispatch, node };
}
...
module.exports = async function testTask0101UnifiedValueSelect(assert) {
  ...
  const searchRowRule = (css.match(/\.value-select-search-row\s*\{[^}]*\}/) || [""])[0];
  const searchRule = (css.match(/\.value-select-search\s*\{[^}]*\}/) || [""])[0];
  const optionRule = (css.match(/\.value-select-options button\s*\{[^}]*\}/) || [""])[0];
  ...
  assert(/position:\s*fixed/.test(popupRule) && /z-index:\s*var\(--layer-dropdown\)/.test(popupRule) && /max-height:\s*min\(280px/.test(popupRule) && /padding:\s*4px/.test(popupRule) && /background:\s*var\(--surface\)/.test(popupRule), "popup must be fixed, white, layered, 4px padded and 280px bounded");
  assert(/height:\s*40px/.test(searchRowRule) && /height:\s*32px/.test(searchRule) && /placeholder='Поиск'/.test(component), "every popup must start with the exact 40px/32px search row");
  ...
  assert(/\.value-select-trigger\[aria-expanded="true"\]::after[\s\S]*?rotate\(180deg\)/.test(css) && /chevron-down-fill-16\.svg/.test(css), "the framed trigger must use the local down arrow and rotate it upward only while open");
  assert(/text-overflow:\s*ellipsis/.test(optionRule + css.match(/\.select-trigger > span\s*\{[^}]*\}/)[0]) && /title='" \+ esc\(option\.label\)/.test(component), "long trigger/option labels must ellipsize visually while retaining their full title");
  ...
  harness.dispatch("click", disabled);
  ...
  harness.dispatch("click", first);
  assert(!harness.popup.hidden && first.getAttribute("aria-expanded") === "true" && harness.document.activeElement === harness.search, "click must open one popup, rotate state and focus its search");
  ...
  harness.search.value = "длинное";
  harness.dispatch("input", harness.search);
  assert(harness.optionsHost.children.length === 1 && harness.optionsHost.children[0].title === "Очень длинное полное название параметра" && selected.length === 0, "search must match the full long label without changing selection");
  harness.search.value = "отсутствует";
  harness.dispatch("input", harness.search);
  ...
  harness.dispatch("keydown", harness.search, { key: "Escape" });
  assert(harness.popup.hidden && harness.document.activeElement === first && selected.length === 0, "Escape must close without change and restore the logical trigger");

  harness.dispatch("keydown", first, { key: "ArrowUp" });
  ...
  harness.dispatch("keydown", harness.search, { key: "Enter" });
  assert(selected.join(",") === "long" && harness.popup.hidden && harness.document.activeElement === first, "Enter must select exactly once, close, and restore focus");
  harness.dispatch("keydown", first, { key: " " });
  assert(!harness.popup.hidden && harness.document.activeElement === harness.search, "Space must open with the same search-first behavior");
  harness.dispatch("click", second);
  ...
  assert(harness.popup.hidden && second.getAttribute("aria-expanded") === "false", "outside pointerdown must close without fabricating a selection");

  harness.dispatch("keydown", second, { key: "Enter" });
  harness.dispatch("keydown", harness.search, { key: "Tab" });
  ...
  harness.dispatch("click", rerender);
  harness.dispatch("click", harness.optionsHost.children[1]);
  assert(rerenderSelection === 1 && replacement && harness.document.activeElement === replacement, "selection must invoke the existing lifecycle exactly once and restore focus after a DOM rerender");
  ...
  assert(expectedSettingKeys.length === 18, "settings selector inventory must contain one Display plot type, eleven backend enums, and six resolution/power-bin modes");
  settingsWindow.SignalAnalyserSettings.setView("display", "time");
