...
module.exports = async function testSignalAddDialog(assert) {
  ...
  assert(/function loadSignalAddCatalog\(\)[\s\S]*?var token = \+\+model\.signalAddToken;[\s\S]*?model\.signalAddCatalog = null;[\s\S]*?model\.signalAddLoading = true;[\s\S]*?renderSignalAddCatalog\(\);[\s\S]*?api\.workspaceVariables\(\)/.test(app), "each catalog request must clear the previous rows, enter loading, render it, then call the authoritative endpoint");
