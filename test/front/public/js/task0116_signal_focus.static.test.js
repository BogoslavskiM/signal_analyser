...
module.exports = async function task0116SignalFocusStatic(assert) {
  ...
  assert(/tab\.textContent=signal\.name;[\s\S]*?if \(!state\.rows\.length && !state\.loading\) loadSignalSamples\(\);/.test(app) && /function showSignalSamples\(\)[\s\S]*?model\.inspectorPage="samples"[\s\S]*?tab\.focus\(\)/.test(app), "Values action/re-entry must target the dynamic tab and load an empty page");
