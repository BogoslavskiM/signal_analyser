...
module.exports=async function(assert){
  ...
  assert(/<th>№ точки<\/th><th>Время<\/th><th>Значение<\/th><th>Модуль<\/th><th>Квадрат<\/th>/.test(app)&&/controller\.reject\(state, request, safeErrorText/.test(load)&&/scroll\.addEventListener\("scroll"[\s\S]*?prefetchSignalSamples\(scroll, state\)/.test(app),"responses render five columns, errors remain visible, and pagination is reusable");
};
