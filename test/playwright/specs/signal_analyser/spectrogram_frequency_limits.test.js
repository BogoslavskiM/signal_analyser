...
function assertSettings(assert, snapshot, expected, label) {
  ...
  assert(value && Object.keys(value).sort().join(",") === "frequency_limits,leakage,overlap_percent", `${label}: spectrogram_settings must contain exactly three keys`);
  assert(same(value, expected), `${label}: unexpected three-key settings ${JSON.stringify(value)}`);
}
...
function expectedSettings(current, frequencyLimits) { return { overlap_percent: Number(current.overlap_percent), leakage: Number(current.leakage), frequency_limits: frequencyLimits }; }
...
async function mutation(page, config, action, log, label, status, expected) {
  ...
  try {
    ...
    if (expected) { const request = body(requests[0], label); if (!same(request.spectrogram_settings, expected) || Object.keys(request.spectrogram_settings || {}).length !== 3) throw new Error(`${label}: request must send the exact full three-key spectrogram_settings body`); }
...
async function setPairByFocusNavigation(page, config, min, max, log, label, expected) {
  ...
  try {
    ...
    if (!response.ok() || requests.length !== 1 || payload.state_revision !== revision + 1 || !same(request && request.spectrogram_settings, expected) || Object.keys(request && request.spectrogram_settings || {}).length !== 3) throw new Error(`${label}: natural pair edit must make one +1 exact three-key /api/view request`);
    await waitForSettled(page, config); return payload;
  } finally { page.off("request", onRequest); }
}
...
async function testSpectrogramFrequencyLimits({ appUrl, assert, config, log, page, step, useCurrentPage }) {
  ...
  try {
    ...
    await step("synthetic 422 and bounded 409 retain exact three-key intent", async function () {
      ...
      assert(same(rejectedBody && rejectedBody.spectrogram_settings, rejected) && Object.keys(rejectedBody.spectrogram_settings || {}).length === 3, "422 must receive an exact full three-key desired body"); assertSettings(assert, await state(page), settings(before), "422 rollback");
