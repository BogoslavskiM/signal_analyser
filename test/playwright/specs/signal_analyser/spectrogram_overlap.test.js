...
async function mutation(page, config, action, log, label, expectedStatus, expectedSettings) {
  ...
  try {
    ...
    if (expectedSettings) {
      ...
      if (!same(request.spectrogram_settings, expectedSettings) || Object.keys(request.spectrogram_settings || {}).length !== 3) throw new Error(`${label}: request must send exact full three-key spectrogram_settings`);
    }
...
async function setOverlap(page, config, value, log, label, expectedStatus, expectedSettings) {
  const input = id(page, config, "spectrogramOverlapPercentInput");
  await input.fill(String(value));
  return mutation(page, config, function () { return input.press("Enter"); }, log, label, expectedStatus, expectedSettings);
}
function assertSettings(assert, snapshot, expected, label) {
  ...
  assert(Object.keys(overlap(snapshot)).sort().join(",") === "frequency_limits,leakage,overlap_percent", `${label}: Spectrogram settings must have exactly three keys`);
...
async function testSpectrogramOverlap({ appUrl, assert, config, log, page, step, useCurrentPage }) {
  ...
  try {
    await step("open Spectrogram and expose explicit default Overlap", async function () {
      ...
      assertSettings(assert, spectrogram, DEFAULT, "default Spectrogram Overlap");
    ...
    await step("valid, equal and local-invalid Overlap lifecycle", async function () {
      const zero = await setOverlap(page, config, 0, log, "set Overlap 0", 200, { overlap_percent: 0, leakage: 0.5, frequency_limits: null }); assertSettings(assert, zero, { overlap_percent: 0, leakage: 0.5, frequency_limits: null }, "Overlap 0"); await waitForHeatmap(page, config, assert, "Overlap 0"); const signature0 = providerSignature(assert, zero, "Overlap 0");
      assert(signature0 !== original.signature50, "deterministic current fixture must change provider x/z after Overlap 50→0");
      const seventyFive = await setOverlap(page, config, 75, log, "set Overlap 75", 200, { overlap_percent: 75, leakage: 0.5, frequency_limits: null }); assertSettings(assert, seventyFive, { overlap_percent: 75, leakage: 0.5, frequency_limits: null }, "Overlap 75"); await waitForHeatmap(page, config, assert, "Overlap 75");
    ...
    await step("synthetic rejection and stale replay restore intended Overlap", async function () {
      ...
      try { await setOverlap(page, config, 0, log, "synthetic Overlap 422", 422, { overlap_percent: 0, leakage: 0.5, frequency_limits: null }); } finally { await page.unroute("**/api/view*"); }
      ...
      await setOverlap(page, config, 75, log, "restore A Overlap 75 after replay", 200, { overlap_percent: 75, leakage: 0.5, frequency_limits: null });
    });
    await step("A/B locality, Clear/re-add and source identity", async function () {
      ...
      original.created = await shell(page, config).getAttribute("data-active-display-id"); await selectPlot(page, config, "spectrogram", log, "open B Spectrogram"); const b = await state(page); assertSettings(assert, b, DEFAULT, "Display B default"); await waitForHeatmap(page, config, assert, "Display B default");
      await selectDisplay(page, config, original.active, log, "return A"); const a = await state(page); assertSettings(assert, a, { overlap_percent: 75, leakage: 0.5, frequency_limits: null }, "Display A local settings");
      const cleared = await mutation(page, config, async function () { await id(page, config, "displayOverflowTrigger").click(); await id(page, config, "clearDisplayAction").click(); }, log, "clear A Spectrogram", 200); assertSettings(assert, cleared, { overlap_percent: 75, leakage: 0.5, frequency_limits: null }, "Clear A");
      ...
      const readded = await mutation(page, config, function () { return page.locator(testIdSelector(member.checkboxTestId)).setChecked(true); }, log, "re-add A signal", 200); assertSettings(assert, readded, { overlap_percent: 75, leakage: 0.5, frequency_limits: null }, "re-add A");
      ...
      assertSettings(assert, switched, { overlap_percent: 75, leakage: 0.5, frequency_limits: null }, "source switch preservation"); assert(wire && wire.signal === candidates[0].name, "Spectrogram wire must keep selected source identity after source switch");
    });
  } finally {
    try {
      ...
      if (original.active) { await selectDisplay(page, config, original.active, log, "cleanup return A"); await selectPlot(page, config, "spectrogram", log, "cleanup Spectrogram"); await restoreMembership(page, config, original.membership, log); await selectSource(page, config, original.source, log, "cleanup analysis source"); const current = await state(page); if (original.settings && !same(overlap(current), original.settings)) await setOverlap(page, config, original.settings.overlap_percent, log, "cleanup Spectrogram Overlap", 200, original.settings); await selectPlot(page, config, original.plot, log, "cleanup original plot"); }
