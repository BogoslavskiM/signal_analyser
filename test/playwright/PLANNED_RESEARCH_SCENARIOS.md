...
|---|---|---|---|---|
| Active-display membership | `layout-geometry`, `frontend-state-management`, `inspector-ui`, `graph-output-zone` | Existing: `plot-card-*`, `active-plot-panel`, `active-plot-title`. Required for membership audit: `active-plot-membership` with one `active-plot-member-*`. | После выбора каждого display существует ровно один membership member, его id/title совпадает с pressed card и active panel; fixed grid сохраняет document geometry. | **Cascade 3 core** |
