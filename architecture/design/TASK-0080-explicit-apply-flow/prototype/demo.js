(function () {
  ...
  state.appliedValues = JSON.parse(JSON.stringify(state.values));
  state.appliedRanges = JSON.parse(JSON.stringify(state.ranges));
  state.appliedResolutionModes = JSON.parse(JSON.stringify(state.resolutionModes));
  ...
  var settingsPanel = document.querySelector("[data-design-id='settings-panel']");
  ...
  var applyAnnouncement = document.querySelector("[data-apply-announcement]");
  ...
  function isPresentationField(fieldId) {
    return ["display.show_legend", "time.normalize_y", "time.show_markers"].indexOf(fieldId) >= 0;
  }

  function isApplyBusy() {
    return state.applyPhase === "applying" || state.applyPhase === "pending";
  }

  function hasLocalInvalidDraft() {
    return Object.keys(state.errors).length > 0;
  }

  function hasDirtyDraft() {
    var valuesDirty = Object.keys(state.values).some(function (fieldId) {
      return !isPresentationField(fieldId) && !sameValue(state.values[fieldId], state.appliedValues[fieldId]);
    });
    var rangesDirty = Object.keys(state.ranges).some(function (fieldId) {
      return !sameValue(state.ranges[fieldId], state.appliedRanges[fieldId]);
    });
    var modesDirty = Object.keys(state.resolutionModes).some(function (fieldId) {
      return !sameValue(state.resolutionModes[fieldId], state.appliedResolutionModes[fieldId]);
    });
    return valuesDirty || rangesDirty || modesDirty;
  }

  function draftValue(fieldId) {
    if (Object.prototype.hasOwnProperty.call(state.ranges, fieldId)) return state.ranges[fieldId].slice();
    if (Object.prototype.hasOwnProperty.call(state.resolutionModes, fieldId)) {
      return { mode: state.resolutionModes[fieldId], value: state.values[fieldId] };
    }
    return state.values[fieldId];
  }

  function persistDraftField(fieldId, reason) {
    var scheduled = state.scheduledDrafts[fieldId];
    if (scheduled && scheduled.timer) clearTimeout(scheduled.timer);
    delete state.scheduledDrafts[fieldId];
    state.backendDraft[fieldId] = JSON.parse(JSON.stringify(draftValue(fieldId)));
    state.fieldUpdates += 1;
    state.eventLog.push({
      type: "field_update",
      field_id: fieldId,
      value: JSON.parse(JSON.stringify(state.backendDraft[fieldId])),
      reason: reason || "debounce_150ms",
      calculation_started: false
    });
  }

  function scheduleDraftField(fieldId) {
    if (isPresentationField(fieldId) || state.errors[fieldId]) return;
    var current = state.scheduledDrafts[fieldId];
    if (current && current.timer) clearTimeout(current.timer);
    state.scheduledDrafts[fieldId] = {
      value: JSON.parse(JSON.stringify(draftValue(fieldId))),
      timer: setTimeout(function () { persistDraftField(fieldId, "debounce_150ms"); }, 150)
    };
  }

  function flushPendingDrafts() {
    var flushed = Object.keys(state.scheduledDrafts);
    flushed.forEach(function (fieldId) { persistDraftField(fieldId, "apply_flush"); });
    return flushed;
  }

  function updateDraftPhase(draftWasEdited) {
    if (isApplyBusy()) return;
    if (!draftWasEdited && (state.applyPhase === "error" || state.applyPhase === "stale")) return;
    if (hasLocalInvalidDraft()) state.applyPhase = "invalid";
    else if (hasDirtyDraft()) state.applyPhase = "dirty";
    else state.applyPhase = "pristine";
    state.applyMessage = "";
  }
  ...
  function currentSemanticMaximum() {
    var range = state.ranges["time.x_limits"] || [];
    return Number(range[1]);
  }

  function announceApply(copy) {
    applyAnnouncement.textContent = "";
    requestAnimationFrame(function () { applyAnnouncement.textContent = copy; });
  }

  function finishApplyAsError(message) {
    state.busy = false;
    state.applyPhase = "error";
    state.applyMessage = message;
    delete state.plotStatus[state.activePane];
    renderPlots();
    renderSettings();
    announceApply(message + " Черновик сохранён.");
    settingsApply.focus();
  }

  function finishApplyAsStale() {
    state.busy = false;
    state.applyPhase = "stale";
    state.applyMessage = "Ответ устарел. Черновик сохранён.";
    delete state.plotStatus[state.activePane];
    renderPlots();
    renderSettings();
    showToast("Ответ устарел — черновик сохранён", "warning");
    announceApply("Ответ устарел. Черновик сохранён. Доступно повторное применение.");
    settingsApply.focus();
  }

  function finishOutputReady() {
    state.busy = false;
    state.outputRevision = state.appliedRevision;
    state.applyPhase = "ready";
    state.applyMessage = "";
    delete state.plotStatus[state.activePane];
    renderPlots();
    renderSettings();
    showToast("График обновлён", "success");
    announceApply("График обновлён. Ревизия " + state.outputRevision + ".");
  }

  function beginApply() {
    if (isApplyBusy() || hasLocalInvalidDraft() || !hasDirtyDraft()) return;
    clearTimeout(applyingTimer);
    clearTimeout(pendingTimer);
    var flushed = flushPendingDrafts();
    state.applyRequests += 1;
    state.eventLog.push({
      type: "apply",
      payload: { display_id: "screen-1", pane_id: state.activePane },
      settings_snapshot_present: false,
      flushed_fields: flushed.slice()
    });
    state.busy = true;
    state.applyPhase = "applying";
    state.applyMessage = "Применяем сохранённый черновик";
    renderSettings();
    announceApply("Применение сохранённого черновика начато.");

    applyingTimer = setTimeout(function () {
      var maximum = currentSemanticMaximum();
      if (Number.isFinite(maximum) && maximum > 1) {
        finishApplyAsError("Сервер отклонил границы: максимум выходит за длительность сигнала.");
        return;
      }
      if (maximum === 0.333 && !state.staleValuesSeen[String(maximum)]) {
        state.staleValuesSeen[String(maximum)] = true;
        finishApplyAsStale();
        return;
      }
      state.appliedRevision += 1;
      state.calculationsCommitted += 1;
      state.applyPhase = "pending";
      state.applyMessage = "Обновляется активная область";
      state.plotStatus[state.activePane] = "loading";
      copyDraftToApplied();
      renderPlots();
      renderSettings();
      announceApply("Настройки приняты. Обновляется только активная область.");
      pendingTimer = setTimeout(finishOutputReady, 650);
    }, 900);
  }
  ...
  function renderPlots() {
    ...
    plotGrid.dataset.outputRevision = String(state.outputRevision);
    plotGrid.dataset.calculationsCommitted = String(state.calculationsCommitted);
  ...
  function renderField(item) {
    ...
    var disabled = !!item.disabled || (!isPresentationField(item.id) && isApplyBusy());
  ...
  function renderApplyChrome() {
    settingsPanel.dataset.applyState = state.applyPhase;
    settingsPanel.setAttribute("aria-busy", isApplyBusy() ? "true" : "false");
    settingsApply.classList.toggle("is-applying", state.applyPhase === "applying");
    settingsApply.classList.toggle("is-pending", state.applyPhase === "pending");
    settingsApply.disabled = isApplyBusy() || hasLocalInvalidDraft() || !hasDirtyDraft();
    settingsApply.textContent = state.applyPhase === "applying" ? "Применение…" : state.applyPhase === "pending" ? "Ожидание…" : (state.applyPhase === "error" || state.applyPhase === "stale") ? "Повторить" : "Применить";
    settingsStatus.textContent = state.applyPhase === "invalid" ? "Исправьте выделенные поля" : state.applyMessage;
  }
  ...
  function renderSettings() {
    ...
    settingsContent.querySelectorAll("[data-field-checkbox]").forEach(function (input) {
      input.addEventListener("change", function () {
        var fieldId = input.getAttribute("data-field-checkbox");
        state.values[fieldId] = input.checked;
        if (isPresentationField(fieldId)) {
          if (fieldId === "display.show_legend") renderPlots();
          return;
        }
        scheduleDraftField(fieldId);
        updateDraftPhase(true);
        renderApplyChrome();
      });
    });
    settingsContent.querySelectorAll("[data-field-input]").forEach(function (input) {
      input.addEventListener("input", function () {
        var fieldId = input.getAttribute("data-field-input");
    ...
    settingsContent.querySelectorAll("[data-range-field]").forEach(function (input) {
      input.addEventListener("input", function () {
        ...
        validateRange(id);
        if (state.errors[id] && state.scheduledDrafts[id]) {
          clearTimeout(state.scheduledDrafts[id].timer);
          delete state.scheduledDrafts[id];
        } else {
          scheduleDraftField(id);
        }
        updateDraftPhase(true);
        renderApplyChrome();
      });
  ...
  function validateRange(id) {
    ...
    if (!pattern.test(String(range[0])) || !pattern.test(String(range[1]))) {
      state.errors[id] = "Введите два числа с точкой как десятичным разделителем.";
      return false;
    }
    var minimum = Number(range[0]);
    var maximum = Number(range[1]);
  ...
  function setSelectValue(key, value) {
    ...
    var resolutionFieldId = key.slice(11);
    state.resolutionModes[resolutionFieldId] = value;
    scheduleDraftField(resolutionFieldId);
    updateDraftPhase(true);
    renderSettings();
  }
  ...
  function hideToast() {
    ...
    if (state.applyPhase === "ready") {
      state.applyPhase = "pristine";
      renderApplyChrome();
    }
  }
  ...
  window.__TASK0080_DESIGN__ = {
    ...
    resetTransient: function () {
      ...
      clearTimeout(applyingTimer); clearTimeout(pendingTimer);
      Object.keys(state.scheduledDrafts).forEach(function (fieldId) {
        clearTimeout(state.scheduledDrafts[fieldId].timer);
      });
      state.scheduledDrafts = {};
      state.busy = false; state.errors = {}; state.plotStatus = {}; state.applyPhase = hasDirtyDraft() ? "dirty" : "pristine"; state.applyMessage = "";
