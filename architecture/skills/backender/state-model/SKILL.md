...
## Workflow
1. Используй inspector-style модель по умолчанию. Отступай от неё только при явном требовании или когда она не соответствует предметной области.
...
- Идентификатор формата сессии хранится под стабильным ключом
  ...
  текущего приложения.
- Для API поверх state используй `backender/api-contract-planning`.
