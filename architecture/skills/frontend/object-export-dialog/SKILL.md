...
## Backend Operation Skill

Все операции реализуются единым `backender/object-export`, который выбирает
режим `workspace`, `julia_script`, `jld2` или `engee_model`. Frontend передаёт
operation id и typed form values, но не формирует domain value и не реализует
формат доставки.
